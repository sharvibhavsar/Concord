...
router.post('/auth/register', async (req: Request, res: Response) => {
  ...
  const [existing] = await db
    .select()
    .from(usersTable)
    .where(and(eq(usersTable.email, email), eq(usersTable.authProvider, 'email')));
...
router.post('/auth/login-email', async (req: Request, res: Response) => {
  ...
  // Find user by email where authProvider = 'email'
  const [user] = await db
    .select()
    .from(usersTable)
    .where(and(eq(usersTable.email, email), eq(usersTable.authProvider, 'email')));
  ...
  // Verify password
  const hash = crypto
    .pbkdf2Sync(password, user.passwordSalt, 100000, 64, 'sha256')
    .toString('hex');
...
router.post('/auth/forgot-password', async (req: Request, res: Response) => {
  ...
  const expiresAt = new Date(Date.now() + 1800000); // 30 min
  ...
  console.log(`[EMAIL SENDING] Password reset link sent to user email: ${email}`);
  console.log(`[EMAIL SENDING] Link: http://localhost:5173/login?reset_token=${token}`);
